...
{
    public partial class frmLibroCxCDiversas : frmBase
    {
        public frmLibroCxCDiversas()
        {
            InitializeComponent();
        }
        Boolean Validar()
        ...
        private void btnLibCaja_Click(object sender, EventArgs e)
        {
            if (Validar())
            {
                ...
                dtslist.Add(new ReportDataSource("dtsCxC", new clsRPTCNContabilidad().CNInvBalCxCDiversa(dFecha)));

                string reportpath = "RptLibroCxCDiversas.rdlc";
