...
{
    public partial class frmLibroProveedor : frmBase
    {
        public frmLibroProveedor()
        {
            InitializeComponent();
        }
        Boolean Validar()
        ...
        private void btnLibCaja_Click(object sender, EventArgs e)
        {
            if (Validar())
            {
                ...
                dtslist.Add(new ReportDataSource("dtsProveedor", new clsRPTCNContabilidad().CNInvBalProveedor(dFecha)));

                string reportpath = "RptLibroProveedor.rdlc";
