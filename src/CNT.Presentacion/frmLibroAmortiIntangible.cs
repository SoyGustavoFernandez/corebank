...
{
    public partial class frmLibroAmortiIntangible : frmBase
    {
        public frmLibroAmortiIntangible()
        {
            InitializeComponent();
        }
        Boolean Validar()
        ...
        private void btnLibCaja_Click(object sender, EventArgs e)
        {
            if (Validar())
            {
                ...
                dtslist.Add(new ReportDataSource("dtsIntangible", new clsRPTCNContabilidad().CNInvBalAmortizacion(dFecha)));

                string reportpath = "RptLibroIntangible.rdlc";
