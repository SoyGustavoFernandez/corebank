...
{
    class clsRptEvalConsumo : clsRptEvalInterface
    {
        ...
        public clsRptEvalConsumo(int idEval, int idSol)
        {
            this.idEvalCre = idEval;
            this.idSolicitud = idSol;
            this.cNombreEval = "CONSUMO";
            this.cnEval = new clsCNEvaluacion();
        }

        public clsRptEvalConsumo(int idEval, int idSol, Form frm)
        {
            this.idEvalCre = idEval;
            this.idSolicitud = idSol;
            this.cNombreEval = "CONSUMO";
        ...
        public void ImpPropuestaEval()
        {
            ...
            if (dsProp.Tables.Count > 0)
            {
                if (this.frmReporteProp == null)
                {
                    ...
                    DataSet dsPropuesta = cnConsumo.CNPropuesta(this.idEvalCre, this.idSolicitud, 2);
                    //DataTable dtExcepciones = cnConsumo.CNRptExcepciones(this.idSolicitud);
                    if (dsPropuesta.Tables.Count > 0)
                    {
                        ...
                        dtsList.Add(new ReportDataSource("dsRcc", dsPropuesta.Tables[6]));
                        dtsList.Add(new ReportDataSource("dsDeudas", dsPropuesta.Tables[7]));
                        dtsList.Add(new ReportDataSource("dsIndicadores", dsPropuesta.Tables[8]));
                        dtsList.Add(new ReportDataSource("dsExcepciones", dsPropuesta.Tables[9]));
                        //dtsList.Add(new ReportDataSource("Excepciones", dtExcepciones));

                        paramList.Add(new ReportParameter("cNomAgen", cNomAgen, false));
                        paramList.Add(new ReportParameter("dFechaSis", clsVarGlobal.dFecSystem.ToString("dd/MM/yyyy"), false));
                        paramList.Add(new ReportParameter("cRutaLogo", clsVarApl.dicVarGen["CRUTALOGO"]));

                        string reportPath = "rptEvalPropConsumo.rdlc";
        ...
        public void ImpEvalCualitativa()
        {
            ...
            if (this.dsCualit.Tables.Count > 0)
            {
                if (this.frmReporteCualit == null)
                {
                    ...
                    DataSet dsEvalCuali = cnEval.CNEvalCualitativa(idEvalCre);
                    if (dsEvalCuali.Tables.Count > 0)
                    {
                        List<ReportParameter> paramList2 = new List<ReportParameter>();
                        List<ReportDataSource> dtsList2 = new List<ReportDataSource>();

                        dtsList2.Add(new ReportDataSource("dsEvalCualita", dsEvalCuali.Tables[0]));
                        dtsList2.Add(new ReportDataSource("dsReferencias", dsEvalCuali.Tables[1]));
                        dtsList2.Add(new ReportDataSource("dsSolicitud", dsEvalCuali.Tables[2]));

                        paramList2.Add(new ReportParameter("cNomAgen", cNomAgen, false));
                        paramList2.Add(new ReportParameter("dFechaSis", clsVarGlobal.dFecSystem.ToString("dd/MM/yyyy"), false));
                        paramList2.Add(new ReportParameter("cRutaLogo", clsVarApl.dicVarGen["CRUTALOGO"]));
                        paramList2.Add(new ReportParameter("cTipoEval", cNombreEval));

                        string reportPath = "rptEvalCualitativa.rdlc";
                        this.frmReporteCualit = new frmReporteLocal(dtsList2, reportPath, paramList2);
