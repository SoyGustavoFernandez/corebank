...
{
    public partial class frmBusCtaAhoVin : frmBase
    {
        #region Variables

        public int idCli, idMon, pTipBus=0, nTipOpe, idestado=1,pnTipoPersona;  //Parametros de Ingreso        
        public string pnCta, pcNroCta, pcEstdo, pcProd, pnMonDisp, pcNomCli, pcXMLInterv;
        public int idProducto;

        #endregion

        public frmBusCtaAhoVin()
        {            
            InitializeComponent();
        }
        ...
        private void frmBusCtaAhoVin_Load(object sender, EventArgs e)
        {
            if (nTipOpe == 9)
            {
                idestado = 4;
            }
            CargarDatos(idCli, idestado, idMon, pnTipoPersona, pcXMLInterv);
        }
        ...
        private void CargarDatos(int nIdCliente, int nEstadoCta, int idMon, int x_nTipPersona,string xmlInterv)
        {
            clsCNDeposito objDeposito = new clsCNDeposito();
            DataTable dtbNumCuentas = objDeposito.CNCuentasAhorrosVin(nIdCliente, nEstadoCta, idMon, x_nTipPersona, xmlInterv);
