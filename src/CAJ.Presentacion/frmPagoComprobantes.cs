...
{
    public partial class frmPagoComprobantes : frmBase
    {
        #region Variables
        ...
        public class datosCpg
        {
            public int idComprobante ;
            public decimal nMontoComprobante ;
            public decimal nTotalOtrosDscts ;
            public decimal nTotalComisiones ;
            public bool lIndAfecRetIGV ;
            public decimal nRetncionIgv ;
            public decimal nImportePagado ;
            public decimal nMontoITF ;
            public int idMoneda ;
            public DateTime dFechaPag ;
            public int idUsuario ;
            public int idAgencia ;
            public int idTipoPago ;
            public int idEntidad ;
            public int idCuentaIFI ;
            public string cNroCheque ;
            public string xmlComisiones ;
            public string xmlAprobador;
            public string cNroCuenta ;
        }

        public datosCpg datCpg = new datosCpg();
        #endregion
        
        #region Eventos

        public frmPagoComprobantes()
        {
            InitializeComponent();
        }
        ...
        private void btnGrabar_Click(object sender, EventArgs e)
        {
            ...
            DataTable result = guardarComprobantePago();
        ...
        private void btnExtorno_Click(object sender, EventArgs e)
        {
            ...
            DataTable result = objCpg.ExtornoPagoComprobantes(idComprobante, idEstadoApro, cSustento, Convert.ToDecimal(txtMontoPagar.Text), clsVarGlobal.dFecSystem,
                                                                clsVarGlobal.User.idUsuario, idMon, idTipoTransac, lModificaSaldoLinea);
        ...
        #region Metodos
        public void getDatosComprobante()
        {
            datCpg.idComprobante = Convert.ToInt32(txtCodigo.Text);
            datCpg.nMontoComprobante  = Convert.ToDecimal(txtTotCompro.Text);
            datCpg.nTotalOtrosDscts = Convert.ToDecimal(txtTotOtrosDesc.Text);
            datCpg.nTotalComisiones = Convert.ToDecimal(txtTotComision.Text);
            datCpg.lIndAfecRetIGV = chcRetIGV.Checked;
            datCpg.nRetncionIgv = Convert.ToDecimal(txtRetIGV.Text);
            datCpg.nImportePagado = Convert.ToDecimal(txtMontoPagar.Text);
            datCpg.nMontoITF = Convert.ToDecimal(txtMontoITF.Text);
            datCpg.idMoneda = Convert.ToInt32(cboMoneda.SelectedValue);
            datCpg.dFechaPag = dtpFechaPago.Value;
            datCpg.idUsuario = clsVarGlobal.User.idUsuario;
            datCpg.idAgencia = clsVarGlobal.nIdAgencia;
            datCpg.idTipoPago = Convert.ToInt32(cboTipoPago.SelectedValue);
            datCpg.idEntidad = Convert.ToInt32(cboEntidad.SelectedValue);
            datCpg.idCuentaIFI = Convert.ToInt32(cboCuenta.SelectedValue);

            string cNroCuenta = "";

            switch (datCpg.idTipoPago)
            {
                case 1:
                    break;
                case 2: //--Transferencias
                    cNroCuenta = cboCuenta.Text;
                    break;
                case 3: //--Cheques Externos
                    cNroCuenta = txtNroCta.Text;
                    break;
            ...
            datCpg.cNroCheque = txtNroCheque.Text.Trim();
            datCpg.xmlComisiones = lstTipoComision.obtenerXml();
            datCpg.xmlAprobador = lstAprobador.obtenerXml();
        }

        public int getEstadoComprobante()
        {
            return Convert.ToInt32(this.dtComprPago.Rows[0]["idEstadoComprobante"]);
        }

        public DataTable guardarComprobantePago()
        {
            int idComprobante = Convert.ToInt32(txtCodigo.Text);
            ...
            DateTime dFechaPag = dtpFechaPago.Value;
            ...
            DataTable result = objCpg.GrabarPagoComprobantes(idComprobante, nMontoComprobante, nTotalOtrosDscts, nTotalComisiones, lIndAfecRetIGV, nRetncionIgv, nImportePagado, nMontoITF, idMoneda,
                                                        ...
                                                        idEntidad, idCuentaIFI, cNroCheque, xmlComisiones, xmlAprobador, cNroCuenta, idTipoTransac, dfechaOpe, lModificaSaldoLinea);
        ...
        public void BuscarComprobante(int idComprobante)
        {
            //=======================================================================
        ...
        public bool validar()
        {
            ...
            {
                 MessageBox.Show("La Fecha de emisión del comprobante, no puede ser mayor a la fecha de pago...Revisar", "Validar Pago de Comprobantes", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                 return true;
            }
