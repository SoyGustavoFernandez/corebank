...
{
    public partial class frmRptVarAcuMensualPresupuesto : frmBase
    {
        ...
        #endregion
        #region Eventos
        public frmRptVarAcuMensualPresupuesto()
        {
            InitializeComponent();
        }
        private void frmRptVarAcuMensualPresupuesto_Load(object sender, EventArgs e)
        {
            verificarPeriodo();
            habilitarImprimir();            
        ...
        private void imprimirReporte()
        {
            ...
            DataTable dtVariacionMensual = rptcnpresupuestos.listarVariacionAcuMensual(nPeriodo, cEstructura, nTipoModificacion);
            dtslist.Add(new ReportDataSource("dsVariacionPresAcuMensual", dtVariacionMensual));
            if (nTipoModificacion == 1)
            {
                string reportpath = "rptVariacionAcuMensualReasig.rdlc";
                new frmReporteLocal(dtslist, reportpath, paramlist).ShowDialog();
            }
            else
            {
                string reportpath = "rptVariacionAcuMensualAmpl.rdlc";
                new frmReporteLocal(dtslist, reportpath, paramlist).ShowDialog();
            }
