...
{
    public partial class frmRptConcProducto : frmBase
    {

        #region Variables Globales

        const string cTituloMsjes = "Reporte de concentración por producto.";
        int nNumReport = 0;
        ...
        private void btnImprimir_Click(object sender, EventArgs e)
        {
            ...
            {
                case 1:
                    reportpath = "rptConcProductoRiesgos.rdlc";
                    paramlist.Add(new ReportParameter("dFecha", dFecha.ToString(), false));
                    cNomRep = dFecha.ToString("yyyy_MM_dd") + " REPORTE DE CONCENTRACIÓN DE CARTERA POR PRODUCTO";
                    dtData = new clsRPTCNRiesgos().CNRptConcProducto(dFecha);
                    dtslist.Add(new ReportDataSource("dsData", dtData));
                    
                    break;
                case 2:
                    dtData = new clsRPTCNRiesgos().CNRptHistConProducto(dFecIni, dFecFin);
                    if (rbtDetalle.Checked)
                    {
                        reportpath = "rptHistConcProductoCapital.rdlc";
                        cNomRep = dFecIni.ToString("yyyy_MM_dd") + "-" + dFecFin.ToString("yyyy_MM_dd") + " REPORTE HISTORICO DE CONCENTRACIÓN DE CARTERA POR PRODUCTO - CAPITAL";
                        dtslist.Add(new ReportDataSource("dsData", dtData));
                    }
                    else
                    {
                        reportpath = "rptGraHistCapProducto.rdlc";
                        cNomRep = dFecIni.ToString("yyyy_MM_dd") + "-" + dFecFin.ToString("yyyy_MM_dd") + " HISTÓRICO CONCENTRACIÓN DE CARTERA POR PRODUCTO - CAPITAL";
                ...
                case 3:
                    ...
                    if (rbtDetalle.Checked)
                    {
                        reportpath = "rptHistConcProductoCapitalMora.rdlc";
                        cNomRep = dFecIni.ToString("yyyy_MM_dd") + "-" + dFecFin.ToString("yyyy_MM_dd") + " REPORTE HISTORICO DE CONCENTRACIÓN DE CARTERA POR PRODUCTO - CAPITAL VENCIDO";
                        dtslist.Add(new ReportDataSource("dsData", dtData));
                    }
                    else
                    {
                        reportpath = "rptGraHistCapVencidoProducto.rdlc";
                        cNomRep = dFecIni.ToString("yyyy_MM_dd") + "-" + dFecFin.ToString("yyyy_MM_dd") + " HISTÓRICO CONCENTRACIÓN DE CARTERA POR PRODUCTO - CAPITAL";
                        dtslist.Add(new ReportDataSource("DataSet1", dtData));
                    }
                    break;
                case 4:
                    ...
                    if (rbtDetalle.Checked)
                    {
                        reportpath = "rptHistConcProductoMora.rdlc";
                        cNomRep = dFecIni.ToString("yyyy_MM_dd") + "-" + dFecFin.ToString("yyyy_MM_dd") + " REPORTE HISTORICO DE CONCENTRACIÓN DE CARTERA POR PRODUCTO - MORA";
                        dtslist.Add(new ReportDataSource("dsData", dtData));
                    }
                    else
                    {
                        reportpath = "rptGraHistIndiceMoraProducto.rdlc";
                        cNomRep = dFecIni.ToString("yyyy_MM_dd") + "-" + dFecFin.ToString("yyyy_MM_dd") + " HISTÓRICO CONCENTRACIÓN DE CARTERA POR PRODUCTO - MORA";
                        dtslist.Add(new ReportDataSource("DataSet1", dtData));
                    }
                    break;
                default:
