...
{
    public class clsGENEjeSP : IntConexion
    {
        #region Listas
        ...
        public DataTable EjecSP(string NombreSP, params object[] parametros)
        {
            ...
            using (SqlConnection Conexion = new SqlConnection())
            {
                try
                {
                    ...
                    Conexion.ConnectionString = clsVarGlobal.IConectionGen.GetConnectionString();// clsGENConexion.leeXML()[0].ToString() ;
        ...
        public List<T> LOEjecutar<T>(string cProcedimiento, params object[] aParametros) where T: new()
        {
            DataTable dtResultado = this.EjecSP(cProcedimiento, aParametros);
            return (dtResultado.Rows.Count > 0) ? dtResultado.ToList<T>() as List<T> : new List<T>();
        }
        public T OEjecutar<T>(string cProcedimiento, params object[] aParametros) where T : new()
        {
            DataTable dtResultado = this.EjecSP(cProcedimiento, aParametros);
            return (dtResultado.Rows.Count > 0) ? dtResultado.Rows[0].ToObject<T>() : new T();
        }
    }
}
