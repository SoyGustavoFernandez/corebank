...
{
    public partial class frmPDPRptNettingReport : frmBase
    {
        #region Variables Globales
        clsCNPdp cnRptPdp = new clsCNPdp();
        DateTime dFechaSystem = clsVarGlobal.dFecSystem;        
        string cTituloMsjes = "Reporte Netting Report";
        #endregion

        #region Eventos
        public frmPDPRptNettingReport()
        {
            InitializeComponent();
        ...
        private void Reporte()
        {
            ...
            DataTable dtData = cnRptPdp.cnNeteo(this.dtpFechaInicial.Value.Date, this.dtpFechaFinal.Value.Date);

            if (dtData.Rows.Count > 0)
            {
                List<ReportDataSource> dtsList = new List<ReportDataSource>();
                dtsList.Add(new ReportDataSource("dsNettingReport", dtData));
                ...
                string reportPath = "rptReporteNettingReport.rdlc";
