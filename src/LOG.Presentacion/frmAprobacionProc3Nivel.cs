...
{
    public partial class frmAprobacionProc3Nivel : frmBase
    {
         #region Variables
        ...
        #region Eventos
        public frmAprobacionProc3Nivel()
        {
            InitializeComponent();
        }

        private void frmAprobacionProc3Nivel_Load(object sender, EventArgs e)
        {
        ...
        private void btnBusNotPed_Click(object sender, EventArgs e)
        {
            ...
            {
                lstNotPed = objCNNotPed.BuscarSolicitudProc3Nivel(0, dtpFechaIni.Value.Date, dtpFechaFin.Value.Date, Convert.ToInt32(cboAgencia.SelectedValue), 1, 4);    
            }
            if (rbtPorFecha.Checked)
            {
                lstNotPed = objCNNotPed.BuscarSolicitudProc3Nivel(0, dtpFechaIni.Value.Date, dtpFechaFin.Value.Date, Convert.ToInt32(cboAgencia.SelectedValue), 1, 3);
            }
        ...
        private void btnAprobar_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Seleccione la nota de pedido a aprobar o denegar.", "Aprobación/Denegación de Solicitudes", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }
            ...
            clsDBResp objDbResp = objCNNotPed.CNAprobarDenegarSolicitudProc3Nivel(objNotPedido);
            if (objDbResp.nMsje == 0)
            {
                MessageBox.Show(objDbResp.cMsje, "Aprobación/Denegación de Solicitudes", MessageBoxButtons.OK, MessageBoxIcon.Information);
                btnAprobar.Enabled = false;
                btnDenegar.Enabled = false;
            ...
            {
                MessageBox.Show(objDbResp.cMsje, "Aprobación/Denegación de Solicitudes", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }           
        }
        ...
        private void dtgListNotPed_SelectionChanged(object sender, EventArgs e)
        {
            ...
            if (dtgListNotPed.SelectedRows.Count > 0 && dtgListNotPed.SelectedRows != null)
            {
                ...
                List<clsDetalleNotaPedido> ItemsNotPed = objCNNotPed.buscarDetalleSolicitudProc3Nivel(objNotPedido.idNotaPedido, lIgv);
        ...
        private void dtgItems_CellValueChanged(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0)
            {
                if (dtgItems.CurrentRow != null)
                {
                    ...
                    {
                        MessageBox.Show("La cantidad atendida no puede ser mayor a la cantidad aprobada.", "Aprobación/Denegación de Solicitudes", MessageBoxButtons.OK, MessageBoxIcon.Warning);
