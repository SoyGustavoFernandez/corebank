...
{
    public partial class frmRptJustificaciones : frmBase
    {
        public frmRptJustificaciones()
        {
            InitializeComponent();
        }

        private void frmRptJustificaciones_Load(object sender, EventArgs e)
        {
        ...
        private void btnImprimir1_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("La Fecha de Inicio no Puede Ser Mayor que la Fecha Final", "Reporte de Justificaciones de Inasistencia", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                return;
            }
            if (cboAgencias1.Text.Trim() == "")
            {
                MessageBox.Show("Debe seleccionar la Agencia", "Reporte de Justificaciones de Inasistencia", MessageBoxButtons.OK, MessageBoxIcon.Information);
                return;
            }
            ...
            DataTable dtRpt = new clsRPTCNRecurHum().CNListarJustificaciones(dFecInicio, dFecFin, idAge );
            if (dtRpt.Rows.Count > 0)
            {
                ...
                dtslist.Add(new ReportDataSource("dsJustificar", dtRpt));
                dtslist.Add(new ReportDataSource("dtsRutaLogo", new clsRPTCNAgencia().CNRutaLogo()));


                string reportpath = "RptJustificaciones.rdlc";

                new frmReporteLocal(dtslist, reportpath, paramlist).ShowDialog();
            }
            else
            {
                MessageBox.Show("No Existen Datos para el Reporte", "Reporte de Justificaciones de Inasistencia", MessageBoxButtons.OK, MessageBoxIcon.Information);
                return;
            }
        }
    }
