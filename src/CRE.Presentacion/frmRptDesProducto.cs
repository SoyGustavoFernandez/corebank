...
{
    public partial class frmRptDesProducto : frmBase
    {
        public frmRptDesProducto()
        {
            InitializeComponent();
        }
        ...
        private void btnImprimir1_Click(object sender, EventArgs e)
        {
            ...
            DataTable dtDesProducto = new clsRPTCNCredito().CNDesembolsoProducto(pdFecIni, pdFecFin, pidAgencia);
            if (dtDesProducto.Rows.Count == 0)
            {
                MessageBox.Show("No existen datos para los parametros seleccionados", "Desembolso Producto", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                return;
            }
            List<ReportDataSource> dtslist = new List<ReportDataSource>();
            dtslist.Clear();
            dtslist.Add(new ReportDataSource("dtsDesembolsoProducto", dtDesProducto));
            ...
            string reportpath = "RptDesembolsoProducto.rdlc";
        ...
        Boolean valida(DateTime dFecIni, DateTime dFecFin)
        {
            if (dFecFin<dFecIni)
            {
                MessageBox.Show("La fecha inicial no puede ser mayor a la final", "Desembolso Producto", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                return true;
            }
            return false;
        }
