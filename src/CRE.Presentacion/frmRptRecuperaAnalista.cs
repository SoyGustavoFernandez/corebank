...
{
    public partial class frmRptRecuperaAnalista : frmBase
    {
        public frmRptRecuperaAnalista()
        {
            InitializeComponent();
        }
        ...
        private void btnImprimir1_Click(object sender, EventArgs e)
        {
            ...
            DataTable dtRecupera = new clsRPTCNCredito().CNRecuperaAnalista(pdFecIni, pdFecFin, pidAgencia);
            if (dtRecupera.Rows.Count == 0)
            {
                MessageBox.Show("No existen datos para los parametros seleccionados", "Recuperaciones", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                return;
            }
            List<ReportDataSource> dtslist = new List<ReportDataSource>();
            dtslist.Clear();
            dtslist.Add(new ReportDataSource("dtsRecuperaAnalista", dtRecupera));
            ...
            string reportpath = "RptEvolRecuperaAnalista.rdlc";
        ...
        Boolean valida(DateTime dFecIni, DateTime dFecFin)
        {
            if (dFecFin<dFecIni)
            {
                MessageBox.Show("La fecha inicial no puede ser mayor a la final", "Recuperaciones", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                return true;
            }
            return false;
        }
