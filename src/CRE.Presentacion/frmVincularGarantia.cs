...
{
    public partial class frmVincularGarantia : frmBase
    {
        ...
        public frmVincularGarantia()
        {
            InitializeComponent();
            conBusCuentaCli.cTipoBusqueda = "S";
            conBusCuentaCli.cEstado = "[0,1,2,13,19]";
        ...
        private void pbxAyuda_Click(object sender, EventArgs e)
        {
            ttpMensaje.Show("Recordar que solo se lista garantías a favor del Cliente Titular" +
                "\n Para agregar registre desde la opcion de Registro de Garantía.", pbxAyuda);
        }
        ...
        private void btnActualizar_Click(object sender, EventArgs e)
        {
            ...
            if (Convert.ToBoolean(rowGarCli.Cells["lReqCtaDep"].Value))
            {
                if (!validarCuentaDeposito((int)rowGarCli.Cells["idGarantia"].Value, Convert.ToDecimal(txtGravamen.Text)))
                    return;                
            }
        ...
        {
            registrarRastreo(Convert.ToInt32(conBusCuentaCli.nIdCliente), 0, "Inicio - Vinculación Garantía", btnGrabar);
            ...
            clsDBResp objDbResp = _objCNGarantia.insertaVinSolGar(idSolicitud, _lstDetGarSolicitud, idUsuario);
            ...
            registrarRastreo(Convert.ToInt32(conBusCuentaCli.nIdCliente), 0, "Fin - Vinculación Garantía", btnGrabar);

            /*  Guardar Expedientes - Estado de Cuenta de Ahorro  */
            clsProcesoExp.guardarCopiaExpediente("Estado de Cuenta de Ahorro", idSolicitud, this);
        }
        ...
        private bool validarCuentaDeposito(int idGarantia, decimal nMontoCoberturar)
        {
            DataTable dtDeposito = _objCNGarantia.CNValidaCuentaDeposito(idGarantia, nMontoCoberturar, Convert.ToInt32(this.conBusCuentaCli.txtNroBusqueda.Text));

            if (Convert.ToInt32(dtDeposito.Rows[0]["lEstado"]) == 0)
            {
                MessageBox.Show(dtDeposito.Rows[0]["cMsgEstado"].ToString(), "Validación de Depósito", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                return false;
            }
            else if (Convert.ToInt32(dtDeposito.Rows[0]["lMonto"]) == 0)
            {
                MessageBox.Show(dtDeposito.Rows[0]["cMsgMonto"].ToString(), "Validación de Depósito", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                return false;
            }
        ...
        {
            if (conBusCuentaCli.nIdCliente == 0 || conBusCuentaCli.nValBusqueda == 0)
                return;
        ...
        private void CargarSolicitud(int idSolicitud)
        {
            ...
            decimal nMontoSolicitado = Convert.ToDecimal(drSol["nCapitalPropuesto"]);
            ...
            txtMonSoli.Text = nMontoSolicitado.ToString("#,0.00");
        ...
        private void SeleccionarGarLista()
        {
            ...
            {
                if (idMonedaDes == 1)
                    nMonGravIni = Math.Round(nMonSaldoGrav * nTipoCambio, 2);
                else
                    nMonGravIni = Math.Round(nMonSaldoGrav / nTipoCambio, 2);
            }
        ...
        private void CalcularPorcentaje()
        {
            ...
            decimal nPorcentaje = (montoAsignar / montoSolicitado) * 100.0M;
