...
{
    public partial class frmRptEvolDesemAnalista : frmBase
    {
        public frmRptEvolDesemAnalista()
        {
            InitializeComponent();
        }
        ...
        private void btnImprimir1_Click(object sender, EventArgs e)
        {
            ...
            DataTable dtDesanalista = new clsRPTCNCredito().CNEvolDesembolsoAnalista(pdFecIni, pdFecFin, pidAgencia);
            if (dtDesanalista.Rows.Count == 0)
            {
                MessageBox.Show("No existen datos para los parametros seleccionados", "Desembolso Analista", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                return;
            }
            List<ReportDataSource> dtslist = new List<ReportDataSource>();
            dtslist.Clear();
            dtslist.Add(new ReportDataSource("dtsDesembolsoAnalista", dtDesanalista));
            ...
            string reportpath = "RptEvolDesembolsoAnalista.rdlc";
        ...
        Boolean valida(DateTime dFecIni, DateTime dFecFin)
        {
            if (dFecFin<dFecIni)
            {
                MessageBox.Show("La fecha inicial no puede ser mayor a la final", "Desembolso Analista", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                return true;
            }
            return false;
        }
