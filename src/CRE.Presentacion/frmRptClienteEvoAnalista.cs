...
{
    public partial class frmRptClienteEvoAnalista : frmBase
    {
        public frmRptClienteEvoAnalista()
        {
            InitializeComponent();
        }
        ...
        private void btnImprimir1_Click(object sender, EventArgs e)
        {
            ...
            DataTable dtEvoCliente = new clsRPTCNCredito().CNSaldoEvolutivoCliente(pdFecIni, pdFecFin, pidAgencia);
            if (dtEvoCliente.Rows.Count == 0)
            {
                MessageBox.Show("No existen datos para los parametros seleccionados", "Evolutivo Clientes", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                return;
            }
            List<ReportDataSource> dtslist = new List<ReportDataSource>();
            dtslist.Clear();
            dtslist.Add(new ReportDataSource("dtsEvoAnalista", dtEvoCliente));
            ...
            string reportpath = "RptEvolClienteAnalista.rdlc";
        ...
        Boolean valida(DateTime dFecIni, DateTime dFecFin)
        {
            if (dFecFin<dFecIni)
            {
                MessageBox.Show("La fecha inicial no puede ser mayor a la final", "Evolutivo Clientes", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                return true;
            }
            return false;
        }
