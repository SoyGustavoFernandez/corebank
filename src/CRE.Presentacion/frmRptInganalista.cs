...
{
    public partial class frmRptInganalista : frmBase
    {
        public frmRptInganalista()
        {
            InitializeComponent();
        }
        ...
        private void btnImprimir1_Click(object sender, EventArgs e)
        {
            ...
            DataTable dtIngresos = new clsRPTCNCredito().CNIngresosAnalista(pdFecIni, pdFecFin, pidAgencia);
            if (dtIngresos.Rows.Count == 0)
            {
                MessageBox.Show("No existen datos para los parametros seleccionados", "Ingresos Analista", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                return;
            }
            List<ReportDataSource> dtslist = new List<ReportDataSource>();
            dtslist.Clear();
            dtslist.Add(new ReportDataSource("dtsEvoAnalista", dtIngresos));
            ...
            string reportpath = "RptEvolIngreAnalista.rdlc";
        ...
        Boolean valida(DateTime dFecIni, DateTime dFecFin)
        {
            if (dFecFin<dFecIni)
            {
                MessageBox.Show("La fecha inicial no puede ser mayor a la final", "Ingresos Analista", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                return true;
            }
            return false;
        }
